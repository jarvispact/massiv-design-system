...
export const shouldForwardProp: ShouldForwardProp = (name, defaultValidator) => (defaultBlacklist.includes(name as string) ? false : defaultValidator(name));

type CreateShouldForwardProp = (blacklist: Array<string>) => (name: React.ReactText, defaultValidator: (p: React.ReactText) => boolean) => boolean;
